...
public class YuushyaDataProvider {
    ...
    public enum DataType{
        ...
        public String getPrefix(Version version){
            if(version.compareTo(Version.V1_21)>=0){
                return switch (this){
                    case LootTable -> "loot_table/blocks/";
                    case Recipe -> "recipe/block/";
                    default -> prefix;
                };
            }
            return prefix;
    ...
    public YuushyaDataProvider version(String version){
    ...
    public ResourceLocation getNewId(){
        return selfPrefix==null||selfPrefix.isEmpty()
                ? ResourceLocation.parse(this.resourceLocation.getNamespace(),this.dataType.getPrefix(this.version)+this.resourceLocation.getPath()+DataType.suffix)
    ...
    public YuushyaDataProvider addTemplateBlockRecipe(YuushyaRegistryData.Block templateBlock){
        ...
        this.json(()-> RecipeData.genStoneCutterRecipe(ResourceLocation.parse(MOD_ID,templateBlock.name) ,getBlueprint("yuushya_template"),1 ,version)).save();
        return this;
    }
    public YuushyaDataProvider addTemplateRecipe(YuushyaRegistryData.Block block,YuushyaRegistryData.Block templateBlock){
        this.id(block.name);
        this.type(DataType.Recipe);
        this.json(()-> RecipeData.genStoneCutterRecipe(ResourceLocation.parse(MOD_ID,block.name) ,ResourceLocation.parse(MOD_ID,templateBlock.name),1 ,version)).save();
        return this;
    }
    ...
    public YuushyaDataProvider add(YuushyaRegistryData.Block block){
        ...
        switch (this.dataType){
            ...
            case Recipe -> {
                ...
                this.json(() -> RecipeData.genStoneCutterRecipe(ResourceLocation.parse(MOD_ID, block.name), getBlueprint(block.itemGroup), getResultNumber(block.itemGroup),version)).save();
            }
            ...
            case Tags -> {
                ...
                for(YuushyaRegistryData.Block.AutoGenerated.Tag tag:block.autoGenerated.tags){
                    ResourceLocation key = TagsData.addTag(tag.type,ResourceLocation.parse(tag.value),ResourceLocation.parse(MOD_ID, block.name),version);
